...
async function info(message) {
    ...
    let [rows] = await global.db.query("SELECT g.str_label, g.str_link, g.og_price, gs.date_start, gs.date_end FROM free_games AS g INNER JOIN free_games_schedule AS gs ON gs.id_game = g.id WHERE g.id = ?;", [argv[1]]);
